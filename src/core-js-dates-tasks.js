...
function getNextFriday(date) {
  const friday = 5;
  const week = 7;
  if (date.getUTCDay() < friday) {
    date.setDate(date.getDate() + friday - date.getUTCDay());
    return date;
  }
  if (date.getUTCDay() === friday) {
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = dEnd.getTime() - dStart.getTime();
  return days / (1000 * 3600 * 24) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let days = 0;
  for (let i = 0; i < endDate.getDate(); i += 1) {
    if (day === 6 || day === 0) {
      days += 1;
    }
    if (day === 6) {
...
function getWeekNumberByDate(date) {
  const stratDate = new Date(date.getFullYear(), 0, 1);
  const days = (date.getTime() - stratDate.getTime()) / (1000 * 3600 * 24) + 1;
...
function getQuarter(date) {
  if (date.getMonth() <= 5 && date.getMonth() > 2) return 2;
  if (date.getMonth() <= 8 && date.getMonth() > 5) return 3;
  if (date.getMonth() <= 11 && date.getMonth() > 8) return 4;
  return 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  function trueDate(str) {
    return new Date(str.slice(6), str.slice(3, 5), str.slice(0, 2));
  }
  const schedule = [];
  const start = trueDate(period.start);
  const end = trueDate(period.end);
